...
include('config.php');
  ...
  if(isset($_POST['add_product'])){
    ...
    if(mysqli_num_rows($select_product_name) > 0){
        ...
        $add_product_query = mysqli_query($conn, "INSERT INTO `automobiles` (imma, marque, prix, annee, km, image)
        VALUES ('$imma', '$marque', '$prix', '$annee', '$km', '$image')")
        or die('query failed');
...
if(isset($_GET['delete'])){
    ...
    $delete_image_query = mysqli_query($conn, "SELECT * FROM `automobiles` WHERE imma = '$delete_id'") or die('query failed');
    $fetch_delete_image = mysqli_fetch_assoc($delete_image_query);
    unlink("uploaded_img/".$fetch_delete_image['image']);
    mysqli_query($conn, "DELETE FROM `automobiles` WHERE imma = '$delete_id'") or die('query failed');
...
mysqli_query($conn, "UPDATE `automobiles` SET imma = '$update_p_imma',
prix = '$update_prix', annee = '$update_annee', km = '$update_km' WHERE imma = '$update_p_imma'")
or die('query failed');
    ...
    if(!empty($update_image)){
        ...
        mysqli_query($conn, "UPDATE `automobiles` SET image = '$update_image' WHERE imma = '$update_p_imma'")
        or die('query failed');
...
<div class="box-container">
    ...
    <?php
        $select_products = mysqli_query($conn, "SELECT * FROM `automobiles`") 
        or die('query failed'); 
    ...
    <div class="box">
        ...
        <a href="admin_products.php?update=<?php echo $fetch_products['imma']; ?>"
        class="option-btn" onclick="return confirm('Vous voulez vraiment modifier les informations de cette voiture?')">Modifier</a>
        <a href="admin_products.php?delete=<?php echo $fetch_products['imma']; ?>"
        class="delete-btn" onclick="return confirm('Supprimer cette voiture?')">Supprimer</a>
...
<section class="edit-product-form">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <input type="text"   name="update_prix" value="<?php echo $fetch_update['prix']; ?>"
        class="box" required placeholder="Le prix de la voiture">
        <input type="text"   name="update_annee" value="<?php echo $fetch_update['annee']; ?>"
        class="box" required placeholder="L'année de la voiture">
        <input type="text"   name="update_km" value="<?php echo $fetch_update['km']; ?>" 
        class="box" required placeholder="Le kilométrage de la voiture" >
        <input type="text"   name="update_marque" value="<?php echo $fetch_update['marque']; ?>" 
        class="box" required placeholder="La marque de la voiture">
